...
function Analytics({ videoLog, setVideoLog, insights, setInsights, bible, finalScripts = [], pendingLogs = [], setPendingLogs, setErr }) {
  ...
  const blank = { title:"", account:"Jayden Gem", platform:"TikTok", views:"", watch:"", completion:"", likes:"", comments:"", dms:"", shares:"", saves:"", celebrity:"", hookType:HOOK_TYPES[0], date:today, nonFollowers:"", female:"", male:"", age1824:"", age2534:"", topCountry:"Myanmar", fyp:"", daysOld:"1", script:"", notes:"", noteImages:[], commentPattern:"", searchTerms:"" };
  ...
  function editEntry(v) {
    ...
    setF({
      ...
      notes:v.notes||"", noteImages:v.noteImages||[],
      commentPattern:v.commentPattern||"", searchTerms:v.searchTerms||""
    });
  ...
  return (
    <div>
      ...
      <Card>
        ...
        {/* Comment Patterns & Search Terms — TikTok API can't read comment text, so log it manually */}
        <div style={{ marginTop:14 }}>
          <div style={{ fontSize:11, color:gold, fontWeight:700, letterSpacing:1.5, marginBottom:6, textTransform:"uppercase" }}>💬 Comments & search patterns</div>
          <div style={{ fontSize:12, color:mute, marginBottom:8, lineHeight:1.5 }}>
            TikTok's API gives comment count but not the actual text. Tell the crew what kind of comments you mostly got and what people searched to find this video — Nami uses this for her analysis.
          </div>
          <div style={{ marginBottom:8 }}>
            <label style={S.fieldLbl}>What kind of comments did you mostly get?</label>
            <textarea style={{ ...S.textarea, minHeight:56 }}
              placeholder='e.g. "mostly asking the price, some saying I look like X celebrity, a few disagreeing with my ranking"'
              value={f.commentPattern} onChange={e=>set("commentPattern",e.target.value)} />
          </div>
          <div>
            <label style={S.fieldLbl}>What did people search to find this video? (if visible in app)</label>
            <textarea style={{ ...S.textarea, minHeight:44 }}
              placeholder='e.g. "Megan Fox jewelry", "celebrity ring price"'
              value={f.searchTerms} onChange={e=>set("searchTerms",e.target.value)} />
          </div>
        </div>

        {/* Notes section — white theme, for cool/unique moments + photos */}
        <div style={{ marginTop:14 }}>
          <div style={{ fontSize:11, color:gold, fontWeight:700, letterSpacing:1.5, marginBottom:6, textTransform:"uppercase" }}>📝 Notes — what made this one special</div>
          <div style={{
            background:"#FFFFFF", borderRadius:12, padding:14, border:"1px solid #E2E2E2",
            boxShadow:"0 2px 10px rgba(0,0,0,0.15)"
          }}>
            <textarea
              style={{
                width:"100%", minHeight:70, background:"#FFFFFF", color:"#1A1A1A",
                border:"1px solid #DDD", borderRadius:8, padding:"10px 12px", fontSize:13.5,
                fontFamily:"inherit", resize:"vertical", outline:"none"
              }}
              placeholder="Did you do something cool or unique in this video? Write it down so you remember for next time…"
              value={f.notes}
              onChange={e=>set("notes",e.target.value)}
            />
            {/* Photo drop for notes */}
            <div style={{ marginTop:10 }}>
              <input type="file" accept="image/*" multiple
                style={{ fontSize:12.5, color:"#444" }}
                onChange={async (e)=>{
                  ...
                  for (const file of files) {
                    try {
                      const data = await new Promise((res,rej)=>{
                        const r = new FileReader();
                        r.onload = ()=>{ const result=String(r.result); const c=result.indexOf(","); res(c!==-1?result.slice(c+1):result); };
                        r.onerror = ()=>rej(new Error("read failed"));
                        r.readAsDataURL(file);
                      });
                      imgs.push({ media_type: file.type||"image/png", data, name: file.name });
                    } catch(err){}
                  }
                  set("noteImages", [...(f.noteImages||[]), ...imgs].slice(0,4));
                  e.target.value = "";
                }}
              />
              {f.noteImages && f.noteImages.length > 0 && (
                <div style={{ display:"flex", gap:8, marginTop:8, flexWrap:"wrap" }}>
                  {f.noteImages.map((img,i)=>(
                    <div key={i} style={{ position:"relative" }}>
                      <img src={`data:${img.media_type};base64,${img.data}`} alt="" style={{ width:60, height:60, objectFit:"cover", borderRadius:6, border:"1px solid #DDD" }} />
                      <button onClick={()=>set("noteImages", f.noteImages.filter((_,j)=>j!==i))}
                        style={{ position:"absolute", top:-6, right:-6, background:"#E0556B", color:"white", border:"none", borderRadius:"50%", width:18, height:18, fontSize:11, cursor:"pointer", lineHeight:1 }}>✕</button>
                    </div>
                  ))}
                </div>
              )}
      ...
      {videoLog.map(v=>(
        <Card key={v.id}>
          ...
          {(v.notes || (v.noteImages && v.noteImages.length>0)) && (
            <div style={{ marginTop:10, background:"#FFFFFF", borderRadius:10, padding:12, border:"1px solid #E2E2E2", boxShadow:"0 2px 8px rgba(0,0,0,0.12)" }}>
              <div style={{ fontSize:10.5, color:"#999", fontWeight:700, letterSpacing:1, marginBottom:5, textTransform:"uppercase" }}>📝 Notes</div>
              {v.notes && <div style={{ fontSize:13, color:"#1A1A1A", lineHeight:1.5 }}>{v.notes}</div>}
              {v.noteImages && v.noteImages.length>0 && (
                <div style={{ display:"flex", gap:6, marginTop:8, flexWrap:"wrap" }}>
                  {v.noteImages.map((img,i)=>(
                    <img key={i} src={`data:${img.media_type};base64,${img.data}`} alt="" style={{ width:54, height:54, objectFit:"cover", borderRadius:6, border:"1px solid #DDD" }} />
                  ))}
                </div>
              )}
            </div>
...
function FrankyExport({ videoLog, finalScripts, ideas, bible, pageInsights = [], setVideoLog, setFinalScripts, setIdeas, setBible, setPageInsights, setInsights, setCalMonth, setCalWeek }) {
  ...
  async function buildExcel() {
    ...
    try {
      ...
      // Tab 1: Video Log
      const vlRows = [["Date","Account","Platform","Celebrity","Title","Format","Views","New Followers","Shares","Avg Watch (s)","Completion %","Likes","Comments","DM Replies","Saves","Non-followers %","Female %","Male %","Age 18-24 %","Age 25-34 %","Top Country","FYP %","Score","Viral","Trend","Lesson","Comment Pattern","Search Terms","Comment Insight","Notes"]];
      for (const v of videoLog) {
        vlRows.push([v.date||"",v.account||"",v.platform||"",v.celebrity||"",v.title||"",v.format||"",v.views||"",v.newFollowers||"",v.shares||"",v.watch||"",v.completion||"",v.likes||"",v.comments||"",v.dms||"",v.saves||"",v.nonFollowers||"",v.female||"",v.male||"",v.age1824||"",v.age2534||"",v.topCountry||"",v.fyp||"",v.ai?.score||"",v.ai?.viral||"",v.ai?.trend||"",v.ai?.summary||"",v.commentPattern||"",v.searchTerms||"",v.ai?.commentInsight||"",v.notes||""]);
      }
      ...
      if (webhookUrl) {
        ...
        } catch(e) { /* webhook failed silently */ }
      }
    } catch(e) {
  ...
  return ContentService.createTextOutput("ok");
            ...
            <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
              ...
              <button style={S.gold} onClick={()=>{
                ...
                if (url) { 
                  try { localStorage.setItem("jg_sheets_webhook", url); } catch(e) {}
                  setWebhookUrl(url);
                  setWebhookInput("");
                }
              }}>Connect</button>
