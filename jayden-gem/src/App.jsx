  ...
  return (
    <div style={S.app}>
      ...
      <div style={S.main}>
        ...
        {tab === "franky" && <FrankyExport videoLog={videoLog} finalScripts={finalScripts} ideas={ideas} bible={bible} />}
...
function Analytics({ videoLog, setVideoLog, insights, setInsights, bible, finalScripts = [], pendingLogs = [], setPendingLogs, setErr }) {
  ...
  function fileToImage(file) {
    return new Promise((res, rej) => {
      ...
      r.onload = () => {
        ...
        // Normalise media_type to what the API accepts
        const allowed = ["image/jpeg", "image/png", "image/gif", "image/webp"];
        const raw = file.type || "image/jpeg";
        const media_type = allowed.includes(raw) ? raw : "image/jpeg";
        res({ media_type, data });
      };
  ...
  async function readShots(fileList) {
    ...
    try {
      ...
      const sys = ctx(bible) + "\nYou are Nami. Read these TikTok/Facebook analytics screenshots carefully. Multiple screenshots may show Overview, Viewers, and Engagement tabs of the same video — combine all numbers. Convert 804K→804000, 1.2M→1200000. Watch time like '15.1s' becomes 15.1. 'Watched full video' = completion. Leave empty string for anything not visible.";
      const usr = `Extract all stats combined. Return ONLY JSON: {"title":"","account":"","platform":"TikTok|Facebook","views":"","watch":"","completion":"","likes":"","comments":"","dms":"","shares":"","saves":"","celebrity":"","date":"","nonFollowers":"","female":"","male":"","age1824":"","age2534":"","topCountry":"","fyp":"","daysOld":""}. Numbers only (no % or units).`;
        ...
        <div style={S.shotBox}>
          ...
          <input
            ...
            onChange={async (e) => {
              ...
              try {
                const images = await Promise.all(files.map(fileToImage));
                await readShots({ length: images.length, [Symbol.iterator]: function*(){ yield* images.map(img=>({ ...img, _preloaded:true })); } });
              } catch(err){ setErr("Couldn't read: "+err.message); }
              setReading(false);
              e.target.value = "";
            }}
          />
...
// ===================== FRANKY EXPORT =====================
function FrankyExport({ videoLog, finalScripts, ideas, bible }) {
  const [busy, setBusy] = useState(false);
  ...
  async function buildExcel() {
    ...
    try {
      ...
      // Download — always same filename so Drive replace works cleanly
  ...
  return (
    <div>
      ...
      <p style={S.lede}>Franky packages everything — your video log, final scripts, ideas, and Brand Bible — into one Excel file. Download it and drag it into Google Drive, replacing the old one. Available Wednesday and Sunday so your Drive stays clean with just one file.</p>
